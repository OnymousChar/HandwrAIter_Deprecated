/*----------------------------------------------------------------------
    NHocr - Japanese OCR     nhocr  ver 0.21
...
/*--------------
  Copyright 2008-2013  Hideaki Goto
...
static char	programTitle[] = "NHocr - Japanese OCR  v0.21";
...
static void usage(){
	fprintf(stderr,"%s  Copyright (C) 2008-2013 Hideaki Goto \n",programTitle);
...
/* ---- Recognize text line(s) ---- */
		...
		NHrec.rec_line(bimage, resultline, MaxLineLength);
		...
		for ( int tlid=0 ; tlid<tlgroup.groups ; tlid++ ){
			...
			NHrec.rec_line(TLimage, resultline, MaxLineLength);
			strcat(resultline, "\n");
			if ( EOF == fputs(resultline,fp_out) ){
				fprintf(stderr,"Can't write on \"%s\".\n",outfile);
				return(2);
