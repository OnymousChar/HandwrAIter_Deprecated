...
int NHrec :: rec_line(SIPImage *image, char *resultline, int bufsize){
	...
	segmentchars(image, cba, cba_raw, \
		&avrcwidth, &lineheight, &charpitch, force_alpha, NHrec::debug);
...
// FIXME with a nice proportional/fixed-width detection!
		...
		if ( cb->nbox == 2 ){
			cid = rec_character(image, &cba_raw[i], recResult1List);
			if ( cid >= 0 ){
...
//				dist2 += recResult1.dist * recResult1.dist;
			...
			cid = rec_character(image, &cba_raw[i+1], recResult2List);
			if ( cid >= 0 ){
...
//			dist2 = sqrt(dist2);
		...
		cid = rec_character(image, cb, recResult_cList);
		if ( cid == -2 )  break;
